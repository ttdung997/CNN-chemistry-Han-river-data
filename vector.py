    ...
    for file in files:
        ...
        for sheet in sheets:
            if sheet != "18b":
                ...
                # print(len(point))
                # print(point)
                attention_point.append(point)
                transformer = FactorAnalysis(n_components=25, random_state=0)
    
    # input_data = transformer.fit_transform(attention_point)
    train_dataset, test_dataset = split_data(attention_point)
    input_data = np.array(input_data)
