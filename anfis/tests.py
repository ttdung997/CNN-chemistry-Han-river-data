...
dictrictArr = ["pH","DO","BOD5","CODMn","TN","TP","TOC","DOC","TN,","NH3-N","NO3-N","DTP","PO4-P"]
...
for dictrict in dictrictArr:
    ...
    ts = numpy.loadtxt("data/"+dictrict+".txt", usecols=range(0, 5))
    X = ts[:,0:4]
    Y = ts[:,4]
    ...
    mf1 = []
    for i in range(0, len(dictrictArr)):
        mf1.append(['gaussmf',{'mean':np.mean(X[i]),'sigma':np.std(X[i+1])}])
