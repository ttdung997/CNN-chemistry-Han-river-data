...
def main():
    ...
    layer_output = transformer.fit_transform(layer_output)
    ...
    for col in full_col:
        print(col)
        output = np.array(output_df[col].values).reshape(len(output_df[col].values),1)
        layer_output

        RegressionData = np.concatenate((layer_output, output), axis=1)
        np.savetxt("fdata/"+str(col)+".txt",RegressionData,delimiter=' ' ,fmt='%1.4e') 
        # break
