...
class LstmAutoencoder(object):
    ...
    def load_model(self, model_dir_path):
        config_file_path = LstmAutoencoder.get_config_file_path(model_dir_path)
        self.config = np.load(config_file_path).item()
        self.threshold = self.config['threshold']

        architecture_file_path = LstmAutoencoder.get_architecture_file_path(model_dir_path)
        self.model = model_from_json(open(architecture_file_path, 'r').read())
        weight_file_path = LstmAutoencoder.get_weight_file_path(model_dir_path)
        self.model.load_weights(weight_file_path)

    def create_model(self, input_dim, encoding_dim):
        model = Sequential()
        model.add(LSTM(200, activation='relu', input_shape=(24,6)))
    ...
    def get_architecture_file_path(model_dir_path):
        return os.path.join(model_dir_path, LstmAutoencoder.model_name + '-architecture.json')

    @staticmethod
    def get_weight_file_path(model_dir_path):
        return os.path.join(model_dir_path, LstmAutoencoder.model_name + '-weights.h5')

    @staticmethod
    def get_config_file_path(model_dir_path):
        return os.path.join(model_dir_path, LstmAutoencoder.model_name + '-config.npy')
