...
class OriginAutoencoder(object):
    model_name = 'OriginAutoencoder'
    ...
    def load_model(self, model_dir_path):
        config_file_path = OriginAutoencoder.get_config_file_path(model_dir_path)
        self.config = np.load(config_file_path).item()
        self.threshold = self.config['threshold']

        architecture_file_path = OriginAutoencoder.get_architecture_file_path(model_dir_path)
        self.model = model_from_json(open(architecture_file_path, 'r').read())
        weight_file_path = OriginAutoencoder.get_weight_file_path(model_dir_path)
    ...
    def get_architecture_file_path(model_dir_path):
        return os.path.join(model_dir_path, OriginAutoencoder.model_name + '-architecture.json')

    @staticmethod
    def get_weight_file_path(model_dir_path):
        return os.path.join(model_dir_path, OriginAutoencoder.model_name + '-weights.h5')

    @staticmethod
    def get_config_file_path(model_dir_path):
        return os.path.join(model_dir_path, OriginAutoencoder.model_name + '-config.npy')
